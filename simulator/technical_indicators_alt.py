...
#fixing the use of floor for indecies
def floor(x):
    return int(x)

#the prices array comes in as [(open_oldest, low_oldest, high_oldest, close_oldest), ..., (open_newest, low_newest, high_newest, close_newest)]

#The functions dont want to be handed the price array - they want to be handed the stock string

#period will be preet in each version
# the function will download a preset amount of data which is all function
...
"""
    ...
    Input:
      prices ndarray
      period int > 1 and < len(prices) (optional and defaults to 21) #this can be changed in the pass arguements
...
def roc_open(price_array):

    #first we need to change the price array to prices

    prices = [e[0] for e in price_array]
...
#    return rocs
  #want to check the last index in the array and return +-1
...
def roc_low(price_array):
    #first we need to change the price array to prices

    prices = [e[1] for e in price_array]
...
def roc_high(price_array):

    #first we need to change the price array to prices

    prices = [e[2] for e in price_array]
...
def roc_close(price_array):

    #first we need to change the price array to prices

    prices = [e[3] for e in price_array]
...
def sma_open(price_array):

    prices = [e[0] for e in price_array]
    period = floor(len(prices)/2)
    num_prices = len(prices)
...
def ema_zero_close(price_array):
...
def ma_env_open(price_array):
    ...
    if ma_envs[-1][4]>0.45 :
        return 1
    elif ma_envs[-1][4]<0.35 :
        return -1
    else:
        return 0

def ma_env_low(price_array):
...
#functions below this are used as reference by other fuctions!
###############################################
...
#Generate a mapping from function name to function, for use in the bot compiler
#funcs = [l for l in locals().keys() if isinstance(locals()[l],types.FunctionType)]
#filter out functions that don't contain 'open close high low'
#funcs = [f for f in funcs if 'open' in f or 'close' in f or 'high' in f or 'low' in f]
#k = locals().keys()
#k.sort()
#print(k)
#func_ref = {f:locals()[f] for f in funcs}

func_ref = {'roc_open':roc_open,
    'roc_close':roc_close,
    'roc_low':roc_low,
    'roc_high':roc_high,
    'rsi_open':rsi_open,
    ...
    'wma_high':wma_high,
    'wma_close':wma_close,
    'ema_zero_open':ema_zero_open,
    'ema_zero_close':ema_zero_close,
    'ema_zero_high':ema_zero_high,
    'ema_zero_low':ema_zero_low,
    'ma_env_open':ma_env_open,
    'ma_env_close':ma_env_close,
    'ma_env_high':ma_env_high,
    'ma_env_low':ma_env_low}
